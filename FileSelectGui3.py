...
def getFilePath(msg):
    ...
    filePath = filedialog.askopenfile(title=msg)  # show an "Open" dialog box and return the path to the selected file
    return filePath
...
def getDirectoryPath(msg):
    ...
    Tk().withdraw()  # we don't want a full GUI, so keep the root window from appearing
    filePath = filedialog.askdirectory(title=msg)  # show an "Open" dialog box and return the path
    return filePath
